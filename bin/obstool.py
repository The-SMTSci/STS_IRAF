...
# class MagnitudesException(Exception):
#     def __init__(self,message,errors=None):
#     @staticmethod
#     def __format__(e):
# class Magnitudes(object):
#     __slots__ = ['U', 'B', 'V', 'R', 'I', 'Z', 'u', 'g', 'r', 'i', 'z', 'w', 'y', 'J', 'H', 'K']
#     def __init__(self, initvals={}):                               # Magnitudes::__init__()
#     def debug(self,msg="",skip=[],os=sys.stderr):           # Magnitudes::debug()
...
__author__  = 'Wayne Green'
__version__ = '0.1'
__all__     = [ 'Magnitudes',  'MagnitudesException'
                'Target',      'TargetException',
                'TargetList',  'TargetListException',
                'Site',        'SiteException'
...
class MagnitudesException(Exception):
    ...
    def __init__(self,message,errors=None):
        super(MagnitudesException,self).__init__("Magnitudes "+ message)
        self.errors = errors
    @staticmethod
    def __format__(e):
        return f" Magnitudes: {e.__str__()}\n"
# MagnitudesException
...
##############################################################################
class Magnitudes(object):
    """ Collection of magnitudes for a target
    UBVRIZ         Johnson Values
    ugriz  wy      Sloans with PanSTARRs wy
    JHK            IR
    Test Data:
mymags = { 'U' : '1.0',  
           'B' : '1.0',
    ...
    def __init__(self, initvals={}):                        # Magnitudes::__init__()
        ...
        initvals a dict with filter name and string/float of value.
        """
        #super().__init__()
        # (wg-python-property-variables)
        self.U   = None        # SIMBAD.allfluxes
        self.B   = None        # SIMBAD.allfluxes
        self.V   = None        # SIMBAD.allfluxes
        self.R   = None        # SIMBAD.allfluxes
        self.I   = None        # SIMBAD.allfluxes
        self.Z   = None        # SIMBAD.allfluxes
        self.u   = None        # SIMBAD.allfluxes
        self.g   = None        # SIMBAD.allfluxes
        self.r   = None        # SIMBAD.allfluxes
        self.i   = None        # SIMBAD.allfluxes
        self.z   = None        # SIMBAD.allfluxes
        self.y   = None        # SIMBAD.allfluxes
        self.J   = None        # SIMBAD.allfluxes
        self.H   = None        # SIMBAD.allfluxes
        self.K   = None        # SIMBAD.allfluxes
        self.w   = None        # PanSTARRs source ? needed ?
        for k,v in initvals.items():
            if(k in self.__dict__):
                try:
                    self.__dict__[k] = float(v)
                except Exception as e:
                    raise MagnitudesException(f"""Magnitude Error. key {k} not handled with slot.""")
    ### Magnitudes.__init__()
    ...
    def debug(self,msg="",skip=[],os=sys.stderr):           # Magnitudes::debug()
        ...
        """
        import pprint
        print("Magnitudes - %s " % msg, file=os)
...
class Target(object):
    ...
    def __init__(self,                                      # Target::__init__()
                 ...
                 simbadname  : str = None,                 # main_id
                 aliases     : str = [],                 # list of aliases
                 references  : str = [],                 # reference(s) for this target in decenting order
                 fluxes      : type[FluxList]
                ):
        """Initialize this class."""
        #super().__init__()
        # (wg-python-property-variables)
...
if __name__ == "__main__":
    ...
    # (wg-python-atfiles)
    elsauce = Site("El Sauce",-70.764941 ,-30.47069,1565.427,+4.0); # PDB-DEBUG
